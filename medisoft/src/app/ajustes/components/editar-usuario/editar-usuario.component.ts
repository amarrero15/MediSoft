...
import { IUsuario } from 'src/app/shared/interfaces/i-usuario';
@Component({
...
export class EditarUsuarioComponent implements OnInit {
  ...
  constructor(private ajustesService:AjustesService,private authService:AuthService,@Inject(MAT_DIALOG_DATA) public data: IUsuario,public dialog: MatDialog) { }
  ...
  editarUsuario(){
    if(this.data.nombre!=''&&this.data.idTipo!=0&&this.data.email!=''){
      this.ajustesService.updateUsuario(this.data).subscribe(res=>{
  ...
  setTipo(){
    ...
    this.data.idTipo=parseInt(value);
  }
